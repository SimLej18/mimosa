#!/usr/bin/env bash
# Run hyperpost across all configurations (mirrors the notebook test loop).
# Each configuration is run in a separate Python subprocess to avoid memory accumulation.
#
# Usage (from project root):
#   bash tests/test_hyperpost_configurations.sh
#
...
PYTHON_SCRIPT="$SCRIPT_DIR/run_hyperpost.py"
