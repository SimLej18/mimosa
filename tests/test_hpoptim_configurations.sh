#!/usr/bin/env bash
# Run HP optimisation (optimise_clusters + update_mixture + optimise_tasks) across all configurations.
# Each configuration is run in a separate Python subprocess to avoid memory accumulation.
#
# Usage (from project root):
#   bash tests/test_hpoptim_configurations.sh
#
...
PYTHON_SCRIPT="$SCRIPT_DIR/run_hpoptim.py"
