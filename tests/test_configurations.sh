#!/usr/bin/env bash
# Run generate_data across all configurations (mirrors the notebook test loop).
...
#
# Fixed: fh=false, siif=true  (commented-out loops in the notebook)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="$SCRIPT_DIR/run_generate_data.py"
